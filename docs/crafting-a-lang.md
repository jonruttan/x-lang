...
- **The line editor is yours to drive, and its colouring is not.**  The
  editor does not install over a lang's `repl`, since it would read Lisp at
  your prompt, so your loop gets it only by asking.  `(Line read prompt)` is
  the door: one edited line back as a string, with the buffer, the cursor,
  the history file and the raw-mode bracketing, none of which carry a
  grammar.  The colouring and the completion do — `Paint` scans for x's `(`,
  `)`, `;` and `"`, and Tab prefix-searches the doc registry against x's
  naming — so install your own with `(Line painter f)` and `(Line completer
  f)`, or pass `()` to either for editing without it.  `(Line available?)`
  says whether there is a terminal to edit on; when there is not, read the
  plain way.
