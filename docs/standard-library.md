...
The x-lang library is modular: 50+ source files organized across `lib/x/boot/`, `lib/x/core/`, `lib/x/type/`, `lib/x/num/`, `lib/x/sys/`, `lib/x/doc/`, `lib/x/tool/`, and `lib/x/platform/`. The bootstrap loader `lib/x-core.x` pre-registers all paths and loads 25 core modules via `provide`/`import` with deduplication.

This document covers the core functions loaded by `lib/x.x` (the base x-lang dialect). For the complete auto-generated reference covering all modules, see [x-lang API Reference](ref/x/index.md) (generated by `make doc-x`).
