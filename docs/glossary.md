...
- **x-expr** (`ext/x-engine-c/ext/x-expr/`) — the expression engine submodule, nested
...
- **the C core** — the interpreter's instruction set (the ISA, cataloged in
  `ext/x-engine-c/tools/contract/isa.x`): evaluator, tokenizer driver, primitives. Deliberately
...
- **ISA fingerprint** — the sha256 of `ext/x-engine-c/tools/contract/isa.x`, the ratcheted
