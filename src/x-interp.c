/** @file x-interp.c
 *  @brief Interpreter object construction, error handling, and environment/IO management.
...
static x_satom_t x_interp_error_hook =
	x_obj_set(NULL, X_OBJ_FLAG_NONE, { .v = (void *)x_interp_error });
 ...
 *
 * @see x_interp_error  -- uses the error-handler from ctrl-group
...
x_obj_t *x_interp_make(x_obj_t *p_base, x_obj_t *p_args)
{
	...
	base_cfg.p_hook_error = (x_obj_t *)x_interp_error_hook;
	...
	x_obj_type(p_base) = x_interp_obj;
	...
	/* Fill env + ctrl (x-expr leaves base.first nil). */
	x_firstobj(x_base(p_base)) = pair(
		/* env: (alist . (local-boundary . (global-tree . shadow-list))) */
		pair(
			/* env-alist */
			pair(nil, nil),
	...
	x_interp_io_state(p_base) = pair(
		/* line */
		pair(atom(1), nil),
		/* booleans: (true . false) */
		pair(
			pair(p_parent ? x_firstobj(x_interp_field_true(p_parent)) : nil, nil),
			pair(p_parent ? x_firstobj(x_interp_field_false(p_parent)) : nil, nil)));
	 ...
	 * The GC hook lists (mark-hooks, free-hooks, mark-roots) now live
	 * in x-expr's heap-group (see x_base_field_heap_*_hooks /
	 * heap_mark_roots), so extras only holds project state that has
	 * nowhere better to go: the eval-roots stack, the token cache, and
	 * the inherited sigint flag pointer. */
	x_interp_extras(p_base) = pair(
		/* eval-list */
		pair(nil, nil),
		pair(
			/* token-cache */
...
x_obj_t *x_interp_type_alist_extend(x_obj_t *p_base, x_obj_t *p_args)
{
	...
	x_restobj((x_obj_t *)args) = x_firstobj(x_interp_field_type_alist(p_base));

	return x_firstobj(x_interp_field_type_alist(p_base)) = x_alist_extend(p_base, (x_obj_t *)args);
}
...
x_obj_t *x_interp_type_alist_assoc(x_obj_t *p_base, x_obj_t *p_args)
{
	...
	x_firstobj((x_obj_t *)args[1]) = x_firstobj(x_interp_field_type_alist(p_base));
...
x_obj_t *x_interp_env_alist_extend(x_obj_t *p_base, x_obj_t *p_args)
{
	...
	x_restobj((x_obj_t *)args) = x_firstobj(x_interp_field_env_alist(p_base));

	return x_firstobj(x_interp_field_env_alist(p_base)) = x_alist_extend(p_base, (x_obj_t *)args);
}
 ...
 *          (via x_interp_buffer_push) and popping it afterward.  Each
 ...
 *
 * @see x_interp_buffer_push -- push buffer before calling
 * @see x_interp_buffer_pop  -- pop buffer after calling
