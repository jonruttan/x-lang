...
; --- Variable expansion ---
(def %sh-expand-word (fn (word)
  (if (not (string? word)) word
    (if (= (string-length word) 0) word
      (if (not (= (char->integer (string-ref word 0)) (char->integer #\$))) word
        (let ((name (substring word 1 (string-length word))))
          (if (string=? name "?") (number->string %sh-status)
            (if (string=? name "$") (number->string %sh-pid)
              (let ((val (sh-getenv name)))
                (if (null? val) "" val))))))))))

(def %sh-expand-words (fn (wds)
  (if (null? wds) ()
    (pair (%sh-expand-word (first wds))
...
(def %sh-setup-redirs (fn (redirs)
  (if (null? redirs) ()
    (do (%sh-setup-redir (first redirs))
        (%sh-setup-redirs (rest redirs))))))

; --- Built-in commands ---
(def %sh-builtin? (fn (name)
  (or (string=? name "echo") (string=? name "cd")
      (string=? name "export") (string=? name "exit")
      (string=? name "true") (string=? name "false")
      (string=? name ":") (string=? name "test")
      (string=? name "["))))

(def %sh-echo (fn (wds)
  (def %print-words ())
  (set %print-words (fn (ws first-word)
    (if (null? ws) ()
      (do (if first-word () (display " "))
          (display (first ws))
          (%print-words (rest ws) ())))))
  (%print-words wds t)
  (newline)
...
(def %sh-cd (fn (wds)
  (let ((dir (if (null? wds)
    ...
    (let ((result (sh-chdir dir)))
      (if (= result -1)
        (do (display "ash: cd: ") (display dir)
            (display ": No such file or directory") (newline) 1)
        0)))))

(def %sh-export (fn (wds)
  (if (null? wds) 0
    (let ((word (first wds)))
      (def %find-eq ())
      (set %find-eq (fn (i)
      ...
      (let ((eq-pos (%find-eq 0)))
        (if (= eq-pos -1) 0
          (do (sh-setenv (substring word 0 eq-pos)
...
(def %sh-run-builtin (fn (name wds)
  (if (string=? name "echo") (%sh-echo wds)
    (if (string=? name "cd") (%sh-cd wds)
      (if (string=? name "export") (%sh-export wds)
        (if (string=? name "exit")
          (sh-exit (if (null? wds) %sh-status
                     (string->number (first wds))))
          (if (string=? name "true") 0
            (if (string=? name "false") 1
              (if (string=? name ":") 0
                (if (string=? name "test") (%sh-test wds)
                  (if (string=? name "[")
                    (let ((test-wds (if (string=? (last wds) "]")
...
(def %sh-run-external (fn (name wds redirs)
  (let ((pid (sh-fork)))
    (if (= pid 0)
      (do (%sh-setup-redirs redirs)
          (sh-exec name wds)
          (display "ash: ") (display name)
          (display ": command not found") (newline)
          (sh-exit 127))
      (sh-wait pid)))))

; --- Assignment handling ---
(def %is-assignment? (fn (word)
  (def %has-eq ())
  (set %has-eq (fn (i)
...
(set %collect-cmd-tokens (fn (cur wds redirs)
  (if (%cursor-empty? cur)
        ...
        (let ((rop (%redir-op? tok)))
          (if rop
            ...
            (if (%tok-is-word? tok)
              ...
              (%sh-run-cmd (reverse wds)
                    (reverse redirs))))))))))

(def %eval-simple-cmd (fn (cur)
  (%collect-cmd-tokens cur () ())))
...
(def %eval-if (fn (cur)
  ...
  (let ((cond-result (%eval-list cur)))
    ...
    (if (= cond-result 0)
      ...
      (do (%skip-body-to-elif-else-fi cur 0)
          (%eval-elif-chain cur))))))

; Skip balanced tokens to elif/else/fi at depth 0
(set %skip-body-to-elif-else-fi (fn (cur depth)
  (if (%cursor-empty? cur)
...
(set %eval-elif-chain (fn (cur)
  (if (%cursor-empty? cur)
    (error "parse error: expected fi")
    (let ((tok (%cursor-peek cur)))
      (if (and (%tok-is-word? tok)
...
(def %eval-while (fn (cur)
  (%cursor-advance! cur) ; consume 'while'
  ; Save position to loop back
  (let ((saved (first cur)))
    (%eval-while-body cur saved))))

(set %eval-while-body (fn (cur saved)
...
(set %collect-for-words (fn (cur ws)
...
(set %eval-for-body (fn (cur var words body-start)
  (if (null? words)
    (do (set %sh-status 0) 0)
    (do (sh-setenv var (first words))
        (set-first cur body-start) ; reset to body
        (let ((result (%eval-list cur)))
          (%skip-newlines cur)
          (%expect-word cur "done")
          (if (null? (rest words))
...
(def %eval-subshell (fn (cur)
  ...
  (let ((pid (sh-fork)))
    (if (= pid 0)
      ...
      (do (%skip-to-close-paren cur 0)
          (let ((status (sh-wait pid)))
            (set %sh-status status) status))))))

(def %skip-to-close-paren (fn (cur depth)
  (if (%cursor-empty? cur)
      ...
      (let ((read-fd (first p))
        ...
        (if (= pid 0)
          ...
          (do (sh-close read-fd)
              (sh-dup2 write-fd 1)
              (sh-close write-fd)
              (let ((cur (%mk-cursor left-tokens)))
                (%eval-command cur))
              (sh-exit %sh-status))
          ; Parent: stdin ← pipe, continue chain
          (do (sh-close write-fd)
              (sh-dup2 read-fd 0)
              (sh-close read-fd)
...
(def %collect-stage ())
(set %collect-stage (fn (cur toks)
  (if (%cursor-empty? cur) (reverse toks)
    (let ((tok (%cursor-peek cur)))
      (if (or (%tok-is-newline? tok)
              (%tok-is-op? tok "|")
              (%tok-is-op? tok ";")
              (%tok-is-op? tok "&")
              (%tok-is-op? tok "&&")
...
(def %collect-stages ())
(set %collect-stages (fn (cur stages)
...
; list: and_or ((';'|'&'|newline) and_or)*
(set %eval-list (fn (cur)
  (%skip-newlines cur)
  (if (%at-stop-word? cur) (do (set %sh-status 0) 0)
              ...
              (if (%match-op cur "&")
                (let ((pid (sh-fork)))
                  (if (= pid 0)
                    ...
                    (do (set %sh-status 0)
                        (%skip-newlines cur)
                        (if (%at-stop-word? cur) 0
                          (%eval-list cur)))))
                result)))))))))

; --- Public API ---
(def sh-eval (fn (input)
