...
(def %sh-word-reader (fn args (mk-tok-word (buffer-token (first args)))))
...
(def %sh-op-reader (fn args (mk-tok-op (buffer-token (first args)))))
...
(def sh-tokenize (fn (input)
  (token-read-string %sh-base input)))
