...
(def %bw-widths (list (list "mark" 512) (list "avatar" 512) (list "banner" 1280)))
(def %bw-width (fn (_ fmt) (first (rest (%find (fn (_ e) (str=? (first e) fmt)) %bw-widths)))))
...
(def %bw-argv
  (fn (_ raw)
    (def ops (List filter (fn (_ a) (not (%bw-engine-flag? a))) (if (pair? raw) (rest raw) ())))
    (if (if (pair? ops) (str=? (first ops) "--") #f) (rest ops) ops)))

(def %bw-rstrip %bw-rstrip-chars)
; index-of answers nil for absent; -1 is easier to compare.
(def %bw-idx (fn (_ sub s) (let ((i (Str8 index-of sub s))) (if (null? i) -1 i))))
(def %bw-ridx (fn (_ sub s) (let ((i (Str8 last-index-of sub s))) (if (null? i) -1 i))))
...
(def %bw-discover
  (fn (_ root)
    ...
    (List for-each
      ...
      (list (list "x-expr" "a C library") (list "x-lang" "the language")))
    (List for-each
      (fn (_ sub)
        (let ((base (%path-join root (first sub))))
          (when (File exists? base)
            (List for-each
              ...
              (%bw-sort (File list-dir base))))))
      (list (list "engines" "an x-lang engine") (list "languages" "a language on x-lang")))
    (%reverse found)))
...
(def %bw-unlink
  (fn (_ t)
    (let go ((s t))
      (let ((j (%bw-idx "](" s)) (j2 (%bw-idx "][" s)))
        (let ((cut (if (< j 0) j2 (if (< j2 0) j (Num min j j2)))))
          (if (< cut 0) s
            (let ((i (%bw-ridx "[" (Str8 sub 0 cut s)))
                  (k (%bw-idx (if (= cut j) ")" "]") (Str8 sub (+ cut 2) (- (Str8 length s) (+ cut 2)) s))))
              (if (if (< i 0) #t (< k 0)) s
                (go (%bw-cat (list (Str8 sub 0 i s)
...
(def %bw-tagline
  (fn (_ readme)
    ...
    (def text0 (Str8 join " " (%bw-words (Str8 replace "`" "" (Str8 replace "_" "" (Str8 replace "*" "" (%bw-unlink (Str8 join " " para))))))))
    (def cut (%bw-earliest text0 (list ". " "; " " -- " " \xe2\x80\x94 ")))
    (def text1 (%bw-rstrip (if (< cut 0) text0 (Str8 sub 0 (+ cut 1) text0)) ".:;, "))
    (def po (%bw-idx "(" text1))
    (def text2
      (if (if (< po 0) #f (< (%bw-idx ")" (Str8 sub po (- (Str8 length text1) po) text1)) 0))
        (%bw-rstrip (Str8 sub 0 po text1) ",;: ")
        text1))
    (if (> (%bw-chars text2) 160)
      (let ((head (%bw-glyph-take text2 160)))
        (let ((sp (%bw-ridx " " head)))
          (%bw-cat (list (%bw-rstrip (if (< sp 0) head (Str8 sub 0 sp head)) ",;: ") "\xe2\x80\xa6"))))
      text2)))
...
(def %bw-png!
  (fn (_ svg-path fmt)
    (def png (%bw-cat (list (Str8 sub 0 (- (Str8 length svg-path) 4) svg-path) ".png")))
    (Proc run! (list "rsvg-convert" "-w" (%bw-s (%bw-width fmt)) svg-path "-o" png))
    png))
      ...
      (List map
        (fn (_ proj)
          (let ((name (first proj)) (dir (first (rest proj))) (kind (first (rest (rest proj)))))
            (let ((tag (%bw-tagline (%path-join dir "README.md"))))
              (let ((p (let go ((fs (list "mark" "avatar" "banner")) (last #f))
                         (if (null? fs) last
                           (let ((r (bitwise-render name (first fs) tag kind "o")))
                             (let ((path (%path-join outdir (%bw-cat (list name "-" (first fs) ".svg")))))
                               (File write-all path (first r))
                               (when (o get 'png) (%bw-png! path (first fs)))
                               (go (rest fs) (rest r))))))))
                (display (%bw-cat (list (Str8 pad-right 16 #\space name) (Str8 pad-right 6 #\space (p get 'opname))
                                        " bit " (%bw-s (p get 'bit)) "  n=" (Str8 pad-left 2 #\space (%bw-s (p get 'n)))
                                        "  hue " (Str8 pad-left 5 #\space (%bw-hue-str (p get 'hue10)))
...
(def %bw-run-one
  (fn (_ o)
    ...
    (if (o get 'json)
      (display (%bw-cat (list (Json emit (rest r)) "\n")))
      (if (o get 'out)
        (do (File write-all (o get 'out) (first r))
            (when (o get 'png) (display (%bw-cat (list (%bw-png! (o get 'out) (o get 'fmt)) "\n")))))
        (display (%bw-cat (list (first r) "\n")))))))

(def bitwise-main
  (fn (_ raw)
    (def o (%bw-opts (%bw-argv raw)))
    (if (o get 'all) (%bw-run-all o)
      (if (o get 'name) (%bw-run-one o)
        (display (%bw-cat (list "usage: x -l bitwise -- NAME [--fmt mark|avatar|banner] [--tagline TEXT] [--kind KIND] [-o FILE] [--png] [--json]\n"
                                "       x -l bitwise -- --all [--root DIR] [--out DIR] [--png]\n")))))))

(provide bitwise/cli bitwise-main %bw-argv %bw-opts %bw-discover %bw-tagline %bw-unlink %bw-idx %bw-ridx %bw-rstrip)
