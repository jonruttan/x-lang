...
**By hand.** The engine (`ext/x-eval-c`), the expression engine it embeds
(`ext/x-eval-c/ext/x-expr`) and the C test runner
...
`ext/x-eval-c` and copies the `x-bin` binary to this repo's root, where the
wrapper and every test runner expect to find it.

The expression engine (`ext/x-eval-c/ext/x-expr`) needs nothing beyond `libc`; the full
