...
# top.  A matching key skips the write; a write takes a few seconds, a load a
# third of one.
#
...
# outside lib/, so the key reads those directories too.  The caller's
# KEY-PATHs come last, in the order given.  An installed tree has no
# tests/x/lib; a directory that is not there hashes as nothing rather than as
# an error.
#
# A KEY-PATH is a bundle's source, and the extensions hashed under it are
# IMG_KEY_EXT, defaulting to .x alone.  A bundle whose modules are written in
# another extension sets it: modules outside the key leave the key unchanged
# when they are edited, so this answers "is current" and the suite runs
# against the library the image holds rather than the one on disk.
#
# A KEY-PATH that names a file is hashed as that file.
_caller_key() {
	for p in "$@"; do
		if [ -f "$p" ]; then
			cat "$p"
...
[ -n "${IMG_CHECK:-}" ] && exit 4
# Only the write needs a wrapper, so one is found here rather than at the top:
# a check answers from the key alone and must not fail for want of an x.
#
# X_SH names the wrapper when the caller knows it.  Callers generally do not
# set it -- the lang bundles that run this from their spec runners do not --
# so the search below has to work from an installed tree as well as a
# checkout, which has no ./x.sh.  Without it the write fails, the failure goes
# to the log beside the image, and the suite boots from source instead.
if [ -z "${X_SH:-}" ]; then
	if [ -f "$root/x.sh" ]; then
		X_SH="$root/x.sh"
...
rm -f "$img" "$keyf" "$skip"
# The writer runs on helium, booted through the wrapper, and that boot is most
# of a write.  X_IMAGE_NO_WRITE keeps the boot from recursing into a write of
# its own: a current image is used, and a stale or absent one is neither
# written nor used, so the writer boots from source only when it has to.
...
# A raise in the writer itself is the writer's bug, and is reported as one.
...
# The writer began and neither a census nor a refusal nor a crash followed, so
# the library ended the writer: an entry that reads stdin at load consumed the
# writer's own script, or exited.  The child is told -- %image-writing is bound
# there -- so a lang that loads and stops while it is bound images like any
# other.
