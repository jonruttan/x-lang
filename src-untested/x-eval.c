...
#define x_extend(BASE, ENV, SYM, VAL) (x_mkspair((BASE), x_mkspair((BASE), (SYM), (VAL)), (ENV)))
*/
...
x_obj_t *x_multiple_extend(x_obj_t *p_base, x_obj_t *env, x_obj_t *syms, x_obj_t *vals)
{
	...
	return syms == nil ? env : x_multiple_extend(p_base, x_extend(p_base, env, x_firstobj(syms),
			x_firstobj(vals)), x_restobj(syms), x_restobj(vals));
}

x_obj_t *x_extend_top(x_obj_t *p_base, x_obj_t *sym, x_obj_t *val)
{
	x_restobj(x_vectorval(p_base, X_I_EXPR)) = x_mkspair(p_base, x_mkspair(p_base, sym, val), x_restobj(x_vectorval(p_base, X_I_EXPR)));
...
{
	return x_restobj(x_assoc(p_base, sym, x_vectorval(p_base, X_I_EXPR)));
}

x_obj_t *x_val_str(x_obj_t *p_base, x_char_t *str)
{
	return x_val_sym(p_base, x_firstobj(x_findsym(p_base, str)));
}
...
x_obj_t *x_mkproccall(x_obj_t *p_base, x_char_t *proc, x_obj_t *exp)
{
	return x_mkspair(p_base,
		x_firstobj(x_findsym(p_base, proc)),
		x_mkspair(p_base,
			x_mkspair(p_base, x_firstobj(x_findsym(p_base, "quote")), x_mkspair(p_base, exp, nil)),
			nil));
}
...
x_obj_t *x_substlis(x_obj_t *p_base, x_obj_t *exps, x_obj_t *env)
{
	...
	return x_mkspair(p_base, x_firstobj(exps), x_substlis(p_base, x_restobj(exps), env));
}
...
x_obj_t *x_evlis(x_obj_t *p_base, x_obj_t *exps, x_obj_t *env)
{
	...
	return x_mkspair(p_base, x_eval(p_base, x_firstobj(exps), env), x_evlis(p_base, x_restobj(exps), env));
}
...
x_obj_t *x_append(x_obj_t *p_base, x_obj_t *exp)
{
	...
	for (args = exp; x_is_type(x_restobj(args), _X_PAIR) && x_restobj(args) != x_nil; args = x_restobj(args)) {
		tmp = x_firstobj(args);

		while(x_is_type(x_restobj(tmp), _X_PAIR) && x_restobj(tmp) != x_nil) {
			tmp = x_restobj(tmp);
		}

		x_restobj(tmp) = x_firstobj(x_restobj(args));
	}

	return x_firstobj(exp);
}
...
eval_start:
	...
	if (is_type(exp, _X_PAIR)) {
		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "quote"))) {
			return x_firstobj(x_restobj(exp));
		}

		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "quasiquote"))) {
			return x_quasiquote(p_base, x_firstobj(x_restobj(exp)), env);
		}


		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "cond"))) {
			while ((exp = x_restobj(exp)) != x_nil) {
				if((tmp = x_eval(p_base, x_firstobj(x_firstobj(exp)), env)) != x_nil) {
					exp = x_restobj(x_firstobj(exp)) != x_nil ? x_firstobj(x_restobj(x_firstobj(exp))) : tmp;
		...
		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, _X_MACRO))) {
			return x_macro(p_base, x_firstobj(x_restobj(exp)), x_restobj(x_restobj(exp)));
		}

		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "lambda"))) {
			return x_proc(p_base, x_firstobj(x_restobj(exp)), x_restobj(x_restobj(exp)), env);
		}

		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "define"))) {
/*			return x_extend_top(p_base, x_firstobj(x_restobj(exp)), x_eval(p_base, x_firstobj(x_restobj(x_restobj(exp))), env));*/
			x_exp = x_extend_top(p_base, x_firstobj(x_restobj(exp)), x_eval(p_base, x_firstobj(x_restobj(x_restobj(exp))), env));

			return exp;

		}

		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "begin"))) {
			exp = x_restobj(exp);
			...
			for (;; exp = x_restobj(exp)) {
				if (x_restobj(exp) == x_nil) {
					exp = x_firstobj(exp);

					goto eval_start;
				}

				x_eval(p_base, x_firstobj(exp), env);
			}
		}

		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "let"))) {
			for (itr = x_firstobj(x_restobj(exp)); itr != nil; itr = x_restobj(itr))
				env = x_extend(p_base, env, x_firstobj(x_firstobj(itr)), x_eval(p_base, x_firstobj(x_restobj(x_firstobj(itr))), env));

			exp = x_mkspair(p_base, x_firstobj(x_findsym(p_base, "begin")), x_restobj(x_restobj(exp)));

			goto eval_start;
		}

		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "set!"))) {
			x_obj_t *pair = x_assoc(p_base, x_firstobj(x_restobj(exp)), env);
			exp = x_eval(p_base, x_firstobj(x_restobj(x_restobj(exp))), env);
			x_restobj(pair) = exp;

			return exp;
		}

		if (x_firstobj(exp) == x_firstobj(x_findsym(p_base, "eval"))) {
			exp = x_eval(p_base, x_firstobj(x_restobj(exp)), env);
#ifdef DEBUG
		...
		proc = x_eval(p_base, x_firstobj(exp), env);

		if (x_is_type(proc, _X_MACRO)) {
			vals = x_substlis(p_base, x_restobj(exp), env);
			exp = x_eval(p_base, x_mkspair(p_base, x_firstobj(x_findsym(p_base, "begin")), x_macrocode(proc)), x_multiple_extend(p_base, env, x_macroargs(proc), vals));
#ifdef DEBUG
		...
		vals = x_evlis(p_base, x_restobj(exp), env);
		...
		if (x_is_type(proc, _X_PROCEDURE)) {
			...
			exp = x_mkspair(p_base, x_firstobj(x_findsym(p_base, "begin")), x_proccode(proc));
...
/*	x_lib_memcpy(p_base->data.p, (x_obj_t*[]){
		...
		p_nil,
		x_mkspair(p_base, x_mkspair(p_base, p_nil, p_nil), p_nil),
		x_mkspair(p_base, p_nil, p_nil),
		x_mkstr(p_base, X_TOKEN_SIZE_MAX),
		NULL
	}, sizeof(x_obj_t*[count + 1]));
*/	p_base->data.p[0] = x_mkint(p_base, count);
	p_base->data.p[X_I_NIL+1] = p_nil;
	p_base->data.p[X_I_EXPR+1] = x_mkspair(p_base, x_mkspair(p_base, p_nil, p_nil), p_nil);
	p_base->data.p[X_I_SYMBOLS+1] = x_mkspair(p_base, p_nil, p_nil);
...
/*
	...
	x_vectorval(p_base, X_I_EXPR) = x_mkspair(p_base, x_mkspair(p_base, nil, nil), nil);
	x_vectorval(p_base, X_I_SYMBOLS) = x_mkspair(p_base, nil, nil);
*/
	p_base = x_make_top();
