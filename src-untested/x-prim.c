...
{
	return x_mkint(p_base, x_obj_prim_length(x_firstobj(args)));
}

x_obj_t *x_prim_not(x_obj_t *p_base, x_obj_t *args)
{
	return x_firstobj(args) == x_nil ? x_firstobj(x_findsym(p_base, "#t")) : x_nil;
}

x_obj_t *x_prim_eq(x_obj_t *p_base, x_obj_t *args)
{
	return x_firstobj(args) == x_firstobj(x_restobj(args)) ? x_firstobj(x_findsym(p_base, "#t")) : x_nil;
}

x_obj_t *x_prim_cons(x_obj_t *p_base, x_obj_t *args)
{
	return x_mkspair(p_base, x_firstobj(args), x_firstobj(x_restobj(args)));
}

x_obj_t *x_prim_car(x_obj_t *p_base, x_obj_t *args)
{
	return x_firstobj(x_firstobj(args));
}

x_obj_t *x_prim_cdr(x_obj_t *p_base, x_obj_t *args)
{
	return x_restobj(x_firstobj(args));
}

x_obj_t *x_prim_num_not(x_obj_t *p_base, x_obj_t *args)
{
	return x_mkint(p_base, ~x_intval(x_firstobj(args)));
}
...
\
	if (args != x_nil)\
		for (result = x_intval(x_firstobj(args)); (args = x_restobj(args)) != x_nil; result op x_intval(x_firstobj(args)));\
\
...
x_obj_t *x_prim_num_##NAME(x_obj_t *p_base, x_obj_t *args)\
{\
	return x_intval(x_firstobj(args)) OP x_intval(x_firstobj(x_restobj(args))) ? x_firstobj(x_findsym(p_base, "#t")) : x_nil;\
}
...
{
	x_pbuf = x_buf;
	for (; args != x_nil; args = x_restobj(args))
		switch(x_firstobj(args)->type) {
		case X_INTEGER:
			x_pbuf += x_lib_strlen(x_lib_strtoint(x_intval(x_firstobj(args)), x_pbuf, 10));
			break;

		case X_CHAR:
			*(x_pbuf++) = x_charval(x_firstobj(args));
			break;

		case X_STRING:
			x_pbuf += x_lib_strlen(x_lib_strncpy(pbuf, x_strval(x_firstobj(args)), X_TOKEN_SIZE_MAX - (x_pbuf - x_buf)));
			break;
...
{
	return x_ownstr(p_base, NULL, x_intval(x_firstobj(args)));
}
...
{
	return x_mkint(p_base, x_lib_strcmp(x_strval(x_firstobj(args)), x_strval(x_firstobj(x_restobj(args)))));
}
...
{
	return x_assoc(p_base, x_firstobj(args), x_firstobj(x_restobj(args)));
}
...
x_obj_t *x_prim_findsym(x_obj_t *p_base, x_obj_t *args) {
	return x_findsym(p_base, x_symname(x_firstobj(args)));
}
...
x_obj_t *x_prim_ptr(x_obj_t *p_base, x_obj_t *args)
{
	...
	for(; args != x_nil; args = x_restobj(args))
		switch(x_firstobj(args)->type) {
		case X_INTEGER:
			x_lib_memcpy(pbuf, &x_intval(x_firstobj(args)), sizeof(x_int_t **));
			pbuf += sizeof(x_int_t *);
			break;

		case X_STRING:
			x_lib_memcpy(pbuf, &x_resval(x_firstobj(x_firstobj(args))), sizeof(x_char_t **));
...
{
	return x_mkint(p_base, x_firstobj(args)->type);
}

x_obj_t *x_prim_totype(x_obj_t *p_base, x_obj_t *args)
{
	return x_gc_obj_make(p_base, (enum x_obj_t_enum)x_intval(x_firstobj(x_restobj(args))), 0, 1, x_firstobj(x_firstobj(args)));
}
