...
- **Object-oriented class system** (`lib/x/type/object.x`) — classes are themselves callable `%class` objects; instances are `%object`. Message-passing dispatch with literal selectors (`(obj name args)`, no quotes — the `call` handler is an operative), single inheritance with `super`, encapsulated mutable fields (external access only via dispatch; method-internal `(field 'f)`/`(set-field! 'f v)` for the private-data pattern), and a `(static …)` block of static methods + class-wide members so a class doubles as a namespace (`(Class name)`, `(Class new …)`). Spec: `tests/x/specs/ext/12-object.spec.md`; guide: `docs/object-system.md`
