...
static char *test_type_int_struct(void)
{
	...
	_it_should("set the Write primitive",
		x_sexp_int_write_prim == x_type_field_write(p_type)
	);
...
static char *test_base_alist_assoc(void)
{
	...
	p_base = x_base_make(NULL, NULL);
	x_base_type_alist_extend(p_base, x_mkspair(p_base, x_type_int_name, atom(1)));
	...
	_it_should("find the type in the Type alist and return its properties",
		x_type_int_name == x_firstobj(p_type)
	);
