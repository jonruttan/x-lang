...
(def-class Iter ()
  (static
    ; WHAT EVERY PUBLIC DOOR ASKS BEFORE IT HANDS `it` TO A DRIVER PRIM.  The
    ; prims dispatch on the type handle, and NIL IS TYPELESS -- there is none
    ; to read, and the process dies.  (A typed non-iterator they survive:
    ; (Iter empty? 5) answered #t.)  So (Iter ->list ()) was a segfault, and
    ; so was every List door on a value with no iter slot, which reaches the
    ; same nil through `new` below.  Checking ONCE on the way in makes a
    ; non-iterator an error instead -- list.x's improper-list guard, same
    ; discipline -- while the drain loops ride `it` through unchanged, so
    ; their per-element prim calls stay unchecked.  Homed on the class rather
    ; than a top-level %-def (the ratchet's classes-are-namespaces rule,
    ; tools/check/percent-globals.sh); sibling calls go through
    ; (Iter %check ...), pin.x's convention.  `Err` resolves at CALL time,
    ; the note at core/list.x's %map1-go.
    ...
    ; so refusing the nil HERE is what makes (List length 5) an error.
