...
(def-class Type ()
  ...
  (static
    ...
    ; namespaces).  Contract rows (tools/contract/base-paths.x) are
      ...
      (match
        ...
        (#t
          (let ((handle t) (raw ((prim-ref (lit type) (lit by-atom)) t)))
            (new Type handle handle raw raw)))))
    (method fields (self)
      (doc "Every type field name in the layout contract (type-rooted rows of tools/contract/base-paths.x)."
        (returns LIST "Field name symbols, contract order"))
      (Type %layout-fields (lit type)))
...
(doc (provide x/type/type Type)
  ...
  (note "(Type wrap t) makes a type interactive: (t name), (t cell 'type-write-stack), (t fields), (t push-write f). Field names come from the layout contract (tools/contract/base-paths.x); (Type fields) lists them.")
