...
(def-class List ()
  (static
    ...
    (method assq (self key alist)
      (doc "Look up a key in an alist by identity (eq?). Returns the (key . val) entry, or nil." (param key ANY "Key to search for") (param alist LIST "Association list"))
      (if (null? alist) ()
        (if (eq? key (first (first alist))) (first alist) (recur self key (rest alist)))))
    (method assoc (self key alist)
      (doc "Look up a key in an alist by equality (equal?). Returns the (key . val) entry, or nil." (param key ANY "Key to search for") (param alist LIST "Association list"))
      (if (null? alist) ()
