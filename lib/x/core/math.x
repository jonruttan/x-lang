...
(def-class Num ()
  (static
    ...
    (method quotient (self (param a INTEGER "Dividend") (param b INTEGER "Divisor"))
      (doc "Integer division truncating toward zero -- always an integer, where / on integers may promote to a rational under the tower." (returns INTEGER "trunc(a/b)")
        (example "(Num quotient -7 2)" "-3"))
      (%int/ a b))
    (method remainder (self (param a INTEGER "Dividend") (param b INTEGER "Divisor"))
      (doc "Truncating remainder (the dividend's sign) -- the pair of quotient; identical to % on integers." (returns INTEGER "a - b*trunc(a/b)")
        (example "(Num remainder -7 2)" "-1"))
      (%int% a b))
    (method modulo (self (param a INTEGER "Dividend") (param b INTEGER "Divisor"))
      (doc "Floored modulo: the result takes the DIVISOR's sign -- (Num modulo -7 3) is 2 where % gives -1." (returns INTEGER "a - b*floor(a/b)")
