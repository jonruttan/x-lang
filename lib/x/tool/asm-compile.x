...
; Stack push/pop ride the 'push/'pop MNEMONICS: each backend owns its
; 16-byte discipline (arm64 pre/post-indexed str/ldr; x86-64 keeps rsp
; 16-aligned for SysV calls at any push depth).
...
(def %asm-compile-mem-set-at
  (fn (_ asm args params)
    ...
    (asm-emit! asm 'push x0)                            ; save address
    (%asm-compile-expr asm (first (rest (rest args))) params) ; value
    (asm-emit! asm 'mov x1 x0)                           ; x1 = value
    (asm-emit! asm 'pop x0)                             ; x0 = address
...
(def %asm-compile-score-set
  (fn (_ asm args params)
    ...
    (asm-emit! asm 'pop x0)                   ; x0 = buffer
    (asm-emit! asm 'mov x2 x0)           ; x2 = buffer
    (asm-emit! asm 'pop x0)                   ; x0 = score
...
(set! %asm-compile-funcall
  (fn (_ asm fn-name args params)
    ...
    (asm-emit! asm 'push x0)                    ; save nil on stack
    (def %build-arg
      (fn (self i)
        (unless (< i 0)
          (do
            ...
            (asm-emit! asm 'pop x0)            ; pop accum -> x0
            ...
            (asm-emit! asm 'pop x0)            ; pop raw arg -> x0
            ...
            (asm-emit! asm 'push x0)           ; push new accum
            (self (- i 1))))))
    (%build-arg (- nargs 1))
    ; Pop final list, prepend nil as self
    (asm-emit! asm 'pop x0)                    ; x0 = (a0 a1 ... aN)
