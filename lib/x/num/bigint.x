...
;
; The #584 blind spot -- a typed pair where BOTH types register the
; operator and NEITHER side's cvt from-alist declares the other -- is
; the ENGINE's to refuse: x_type_op_try raises the teaching error
; naming both types instead of falling through to raw payload-word
; reads.  One arbitration, one raise, every operator door (+ - * / %
; < =); the interim lib-side guard this file briefly carried is gone
; with it.
