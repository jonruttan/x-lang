...
(def sh-setenv (fn (name val) (ptr-call %c-setenv name val 1)))

; --- getenv: returns char*, convert to string (or () if NULL) ---

(def sh-getenv (fn (name)
...
(def sh-open-write (fn (path) (ptr-call %c-open path 1537 438)))
(def sh-open-append (fn (path) (ptr-call %c-open path 521 438)))
      ...
      (let ((argv (int->ptr (ptr-call %c-malloc (* (+ n 1) 8)))))
        (def %fill (fn (lst i)
          (if (null? lst)
            ...
            (do (ptr-set-word! argv (* i 8) (ptr->int (string->ptr (first lst))))
                (%fill (rest lst) (+ i 1))))))
        (%fill all 0)
