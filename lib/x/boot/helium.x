...
;
; Shared by the lib/he.x entry and the lib/x.x default pointer (#95).  The
...
; EOF and exits instead of reading the session's stdin.  Both entries
; therefore keep the launcher at stream top level and include this body.
; (Same extraction idiom as boot/tower-compiled.x.)
...
(%set-first! %module-loaded-cell
  (pair (lit x/boot/helium) (first %module-loaded-cell)))
