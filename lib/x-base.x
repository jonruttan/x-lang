...
;   numeric tower.  Mirrors x-and.x's analyser-compilation (see docs/dialects.md
;   and docs/type-system.md) but without the interactive banner/REPL, so the
;   tower's tokenizer analysers run as native compiled code instead of
;   interpreted closures (~20x faster tokenizing of subsequent source/tests).
...
; Pre-register all heavy module paths so internal imports are no-ops
